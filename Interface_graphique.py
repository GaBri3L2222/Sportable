...
class Interface_graphique(metaclass=Singleton):
    ...
    def add_exercise(self, exercise_name, reps, sets):
        ...
        try:
            # Pass exercise data as JSON string to the service
            result = igs.service_call("Moteur", "addExercice", json.dumps(exercise_data), "")
            exercise_data["id"] = result
        except Exception as e:
            print(f"Error calling addExercice service: {e}")
    ...
    def _notify_frontend(self):
        """Send the current exercises and index to the web frontend (if available)."""
        try:
            ...
            js = f"if(window.setExercisesFromPython) window.setExercisesFromPython({payload});"
            # evaluate_js is synchronous in many pywebview builds; errors are fine to catch
            try:
                self.window.evaluate_js(js)
            except Exception as e:
                # Fallback: try via webview.windows[0] if available
                try:
                    import webview as _wv
                    if _wv.windows:
                        _wv.windows[0].evaluate_js(js)
                except Exception:
                    print('Could not notify frontend:', e)
        except Exception as e:
            print('Error in _notify_frontend:', e)
    ...
    def start_workout(self):
        ...
        if len(self.exercises) > 0:
            ...
            try:
                ...
                pass
            # Notify frontend that session started
            self._notify_frontend()
    ...
    def stop_workout(self):
        ...
        try:
            ...
            pass
        # Notify frontend that session stopped
        self._notify_frontend()
    ...
    def add_rest(self, duration_seconds: int):
        ...
        try:
            # Pass rest duration data as JSON string to the service
            result = igs.service_call("Moteur", "addRecuperation", json.dumps(rest_data), "")
            rest_data["id"] = result
        except Exception as e:
            print(f"Error calling addRecuperation service: {e}")
            rest_data["id"] = None
            
        self.exercises.append(rest_data)
        # Notify frontend
        self._notify_frontend()
    ...
    def Settotaldisplay(self, sender_agent_name, sender_agent_uuid, Displayjson):
        ...
        try:
            ...
            # Notify frontend of new data pushed by Moteur
            self._notify_frontend()
        except:
            pass
...
class Api:
    ...
    def get_execution_state(self):
        ...
        current_item = None
        if 0 <= interface.current_exercise_index < len(interface.exercises):
            current_item = interface.exercises[interface.current_exercise_index]
        
        # Priority 1: Motor provides rest time
        if interface.Rest_Time_RemainingI is not None:
            try:
                rest_time = int(interface.Rest_Time_RemainingI)
            except:
                rest_time = 0
            # Reset local timer when motor provides value
            interface.local_rest_seconds = None
            interface._last_rest_item_key = None
            
        # Priority 2: Local timer for pause items (when motor doesn't provide)
        elif current_item and current_item.get("type") == "pause":
            duration = current_item.get("duree_secondes", 0)
            current_key = (interface.current_exercise_index, duration)
            
            # Initialize or reset timer for new pause item
            if interface._last_rest_item_key != current_key:
                interface._last_rest_item_key = current_key
                interface.local_rest_seconds = duration
                rest_time = duration
            elif interface.local_rest_seconds is not None and interface.local_rest_seconds > 0:
                # Decrement local timer
                interface.local_rest_seconds -= 1
                rest_time = interface.local_rest_seconds
                
                # Trigger fin_timer output when timer reaches 0
                if interface.local_rest_seconds <= 0:
                    interface.set_Fin_TimerO()
                    interface.local_rest_seconds = None
            else:
                rest_time = 0
        else:
            # Not a pause item, reset local timer
            interface.local_rest_seconds = None
            interface._last_rest_item_key = None
